...
func StartScraping() error {
	...
	unscraped_urls, err := services.GetUnscrappedUrlFrontiers(ctx, 1)
	if err != nil {
		return err
	}
...
func scrapeUrls(urlFrontiers []repository.UrlFrontier) ([]repository.Extraction, error) {
	...
	for _, frontier := range urlFrontiers {
		scrapeUrl(frontier, &extractions)
	}
...
func scrapeUrl(frontier repository.UrlFrontier, extractions *[]repository.Extraction) {
